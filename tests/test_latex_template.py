...
class TestSmartEscapeDollars(unittest.TestCase):
    """Тесты для функции smart_escape_dollars"""
    
    def test_inline_math_formula(self):
        """Тест: inline math формулы не должны экранироваться"""
        text = "$C(t)$ -- концентрация в крови"
        result = smart_escape_dollars(text)
        self.assertEqual(result, "$C(t)$ -- концентрация в крови")
    
    def test_multiple_inline_math_formulas(self):
        """Тест: несколько inline math формул не должны экранироваться"""
        text = "$C_0$ -- начальная доза, $k$ -- константа"
        result = smart_escape_dollars(text)
        self.assertEqual(result, "$C_0$ -- начальная доза, $k$ -- константа")
    
    def test_math_with_superscript(self):
        """Тест: математические формулы со степенями не должны экранироваться"""
        text = "мин$^{-1}$ для взрослых"
        result = smart_escape_dollars(text)
        self.assertEqual(result, "мин$^{-1}$ для взрослых")
    ...
    def test_no_dollars(self):
        ...
        result = smart_escape_dollars(text)
        self.assertEqual(result, "Обычный текст без долларов")
